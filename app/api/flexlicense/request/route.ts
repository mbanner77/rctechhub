...
export async function POST(req: NextRequest) {
  try {
    ...
    const siteCfg = await getSiteConfig().catch(() => ({ currency: "EUR" as const }))
    ...
    const html = `
      ...
      <hr/>
      <p><b>Projektvolumen:</b> ${formatCurrency(Number(body?.projectVolume||0), siteCfg.currency)}</p>
      ...
      <hr/>
      <p><b>Monatlich:</b> ${formatCurrency(Number(body?.results?.monthly||0), siteCfg.currency)}</p>
      <p><b>Gesamt:</b> ${formatCurrency(Number(body?.results?.total||0), siteCfg.currency)}</p>
      <p><b>NPV @10%:</b> ${formatCurrency(Number(body?.results?.npv||0), siteCfg.currency)}</p>
